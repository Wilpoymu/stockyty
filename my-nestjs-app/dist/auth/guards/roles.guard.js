...
let RolesGuard = RolesGuard_1 = class RolesGuard {
    ...
    canActivate(context) {
        ...
        if (!user) {
            ...
            throw new common_1.ForbiddenException('No user information available');
        }
        const hasRole = this.checkRoles(user, requiredRoles);
        if (!hasRole) {
            this.logger.warn(`User ${user.username || user.id} does not have required roles`);
        }
        return hasRole;
    }
    checkRoles(user, requiredRoles) {
        ...
        if (Array.isArray(user.roleUsers) && user.roleUsers.length > 0) {
            return requiredRoles.some(role => user.roleUsers.some(roleUser => roleUser.role?.name === role || roleUser.roleId === role));
        }
        this.logger.error(`Could not find roles in user object: ${JSON.stringify(user)}`);
        return false;
    }
};
